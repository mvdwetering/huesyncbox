...
async def test_unload_entry(hass: HomeAssistant, mock_api: Mock) -> None:
    integration = await setup_integration(hass, mock_api)
...
async def test_migrate(hass: HomeAssistant, mock_api: Mock) -> None:
    # Create v1 entry
...
async def test_migrate_v1_to_v2(hass: HomeAssistant) -> None:
    ...
    mp_entity = entity_registry.async_get_or_create(
        "media_player",
        huesyncbox.DOMAIN,
    ...
    entity_registry = er.async_get(hass)
    automation_entity = entity_registry.async_get_or_create(
        "automation",
        huesyncbox.DOMAIN,
    ...
    # Check results
    assert entity_registry.async_get(mp_entity.entity_id) is None


async def test_migrate_v2_1_to_v2_2(hass: HomeAssistant) -> None:
    # Create v1 entry
    mock_config_entry = MockConfigEntry(
        version=2,
        domain=huesyncbox.DOMAIN,
